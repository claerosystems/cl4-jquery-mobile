...
class Controller_CL4_CL4Admin extends Controller_Private {
	...
	*/
	...
	protected $no_auto_render_actions = array('download', 'export', 'lookup');
	...
	public function before() {
		...
		if ( ! $this->check_perm($perm_action)) {
			...
			if ($action != 'index') {
				...
				$this->set_session();
				$this->redirect(Base::get_url('cl4admin', array('action' => 'index', 'model' => $default_model)));
			} else {
				$this->set_session();
				$this->redirect(Base::get_url('login', array('action' => 'noaccess')) . URL::array_to_query(array('referrer' => $this->request->uri()), '&'));
			}
	...
	*/
	public function action_add() {
		...
		if ( ! empty($this->id)) {
			...
			$form_options['form_action'] = Base::get_url('cl4admin', array('model' => $this->model_name, 'action' => 'add')) . URL::query();
			//URL::site($this->request->route()->uri(array('model' => $this->model_name, 'action' => 'add'))) . URL::query();
		}
	...
	*/
	...
	public function action_export() {
		...
		$export_result = $orm_multiple->get_export(array('use_db_values' => FALSE));

		//echo Debug::vars($export_result);
	...
	/**
	 * Used by ajax suggest fields to provide a list of options as the user types.
	 */
	public function action_lookup() {
		...
		$return_data = array(
			'status' => TRUE,
			'status_message' => 'The query was successful.',
			'data' => array(),
		);
